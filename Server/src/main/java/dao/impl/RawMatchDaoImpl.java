...
public class RawMatchDaoImpl implements RawMatchDao {
	...
	public List<Match> getAllMatch() {
		...
		if (seasons==null) return list;
		for (String s : seasons) {
			if (s.startsWith(".")) continue;
			String path = MATCH_PATH + s + "/";
			File fs = new File(path);
			if (!fs.isDirectory())
				continue;
			String[] names = fs.list();
			String season = s.substring(2, 5)
					+ s.substring(7);
	...
	private Match getEntity(String season, String name, String path) {
		...
		List<MatchPlayerBasic> home_basics = new ArrayList<>();
		List<MatchPlayerBasic> guest_basics = new ArrayList<>();
		List<MatchPlayerAdvanced> home_advanced = new ArrayList<>();
		List<MatchPlayerAdvanced> guest_advanced = new ArrayList<>();
		...
		for (int i = 0; i < lines.size(); ++i) {
			...
			case 0:
				isStarter++;
				guest_basics.add(getBasic(checkId, match.getGuest_team(),
						lines.get(i), isStarter));
				break;
			case 1:
				isStarter++;
				home_basics.add(getBasic(checkId, match.getHome_team(), lines.get(i), isStarter));
				break;
			case 2:
				isStarter++;
		...
		match.setHome_basic_list(home_basics);
		match.setHome_advanced_list(home_advanced);
		match.setGuest_basic_list(guest_basics);
