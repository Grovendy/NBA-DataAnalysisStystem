...
public class InitDaoImpl implements InitDao {
	...
	public void dropTable() {
        sqlManager.getConnection();
        sqlManager.executeUpdate("drop table if exists season");
        sqlManager.executeUpdate("drop table if exists match_info");
        sqlManager.executeUpdate("drop table if exists match_score");
        sqlManager.executeUpdate("drop table if exists match_player_basic");
        sqlManager.executeUpdate("drop table if exists match_player_advanced");
        sqlManager.executeUpdate("drop table if exists player_info");
        sqlManager.executeUpdate("drop table if exists player_total");
        sqlManager.executeUpdate("drop table if exists player_per_game");
        sqlManager.executeUpdate("drop table if exists player_advanced");
        sqlManager.executeUpdate("drop table if exists player_salary");
        sqlManager.executeUpdate("drop table if exists team_info");
        sqlManager.executeUpdate("drop table if exists team_total");
        sqlManager.executeUpdate("drop table if exists team_per_game");
        sqlManager.executeUpdate("drop table if exists team_advanced");
        sqlManager.executeUpdate("drop table if exists team_opp_total");
        sqlManager.executeUpdate("drop table if exists team_opp_per_game");
        sqlManager.releaseConnection();
	}
	...
	public void fillTable() {
		...
		List<TeamInfo> allTeamInfo = rawTeam.getAllTeamInfo();
		Iterator<TeamInfo> iterator = allTeamInfo.iterator();
		HashSet<String> set = new HashSet<>(allTeamInfo.size());
		while (iterator.hasNext()) {
			TeamInfo next = iterator.next();
			if (set.contains(next.getAbbr())) {
				iterator.remove();
			}
			set.add(next.getAbbr());
		}
		team.insertTeamInfo(allTeamInfo);
